...
public class PrePostProcessor {
    ...
    // model output is of size 25200*21
    private static int mOutputRow = 25200;
    private static int mOutputColumn = 21;

//    private static int mOutputRow = 67200; // as decided by the YOLOv5 model for input image of size 640*640
    ...
    static ArrayList<Result> outputsToNMSPredictions(float[] outputs, float imgScaleX, float imgScaleY, float ivScaleX, float ivScaleY, float startX, float startY) {
        ...
        for (int i = 0; i< mOutputRow; i++) {
            if (outputs[i* mOutputColumn +4] > mThreshold) {
                ...
                Result result = new Result(cls, outputs[i*21+4], rect);
                results.add(result);
            }
        }
